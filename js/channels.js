...
export function getChannelsByAgeAndCategory(ageGroup, weakCategories = [], maxResults = 3) {
  ...
  // 3차: 여전히 부족하면 나이대 ± 1단계 확장 (초5-6은 초3-4로 통합)
  if (matched.length < maxResults) {
    const AGE_ORDER = ['유아', '초1-2', '초3-4'];
    const normalizedAge = ageGroup === '초5-6' ? '초3-4' : ageGroup;
    const idx = AGE_ORDER.indexOf(normalizedAge);
...
export function getChannelsByAge(ageGroup, maxResults = 3) {
  const normalizedAge = ageGroup === '초5-6' ? '초3-4' : ageGroup;
  return CHANNELS
    .filter(ch => ch.age === normalizedAge && ch.stars >= 4)
    .sort((a, b) => {
      // 유아: 동요/챈트 우선 정렬
      if (normalizedAge === '유아') {
        const aIsNursery = a.genre === '동요/챈트' ? 1 : 0;
